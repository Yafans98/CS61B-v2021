...
public class Model extends Observable {
  ...
  public boolean tilt(Side side) {
    ...
    for (int col = 0; col < b.size(); col += 1) {
      ...
      for (int row = size() - 1; row >= 0; row -= 1) {
        ...
        Tile below = this.board.tile(col, row - 1);
        if (current == null || below == null) {
          break;
        }
        int cVal = current.value();
        int bVal = below.value();
        if (cVal == bVal) {
          board.move(col, row, below);
          score += cVal * 2;
          for (int L = row - 2; L >= 0; L--) {
            Tile afterMerge = board.tile(col, L);
            if (afterMerge == null) {
              break;
            }
            if (L < size()) {
              board.move(col, L + 1, afterMerge);
            }
          }
          changed = true;
        }
  ...
  public static boolean atLeastOneMoveExists(Board b) {
    ...
    Tile rightTile;
    int bSize = b.size();
    for (int col = 0; col <= bSize - 1; col += 1) {
      for (int row = 0; row <= bSize - 2; row += 1) {
        Tile current = b.tile(col, row);
        if (current == null) {
          return true;
        }
        upTile = b.tile(col, row + 1);
        if (col != bSize - 1) {
          rightTile = b.tile(col + 1, row);
          if (upTile == null || rightTile == null) {
            return true;
          } else {
            if (current.value() == upTile.value() || current.value() == rightTile.value()) {
              return true;
            }
          }
        }
        if (current.value() == upTile.value()) {
          return true;
        }
      }
    }
